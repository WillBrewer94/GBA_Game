...
void drawBackgroundImage4(volatile const unsigned short* image) {
    DMANow(3, image, videoBuffer, ((240 * 160)/2) | (0 << 23));
}
...
void drawRect4(int row, int col, int height, int width, unsigned char colorIndex) {
 ...
 for(int r = 0; r < height; r++) {
  ...
  if(col & 1 && !(width & 1)) {
   setPixel4(row + r, col, colorIndex);
   setPixel4(row + r, col + width - 1, colorIndex);
   DMANow(3, &pixels, &videoBuffer[((row + r)*(240)+(col + 1))/2], (width - 1)/2 | (2 << 23));
  }
  if(!(col & 1) && !(width & 1)) {
...
void drawImage4(volatile const unsigned short* image, int row, int col, int height, int width) {
 for(int r = 0; r < height; r++) {
  DMANow(3, &image[((r)*(width)+(0))/2], &videoBuffer[((row + r)*(240)+(col))/2], width/2 | (0 << 23));
 }
}
...
void loadPalette(volatile const unsigned short* palette) {
    DMANow(3, palette, ((u16 *)0x5000000), 256 | (0 << 23));
}
...
void loadMap(const unsigned short * map, unsigned short mapLen, unsigned short palIndex, unsigned short sbb) {
 ...
 int i;
 for(i = 0; i < mapLen; i++) {
  unsigned short maskedNum = map[i] & ~(((0xF)<<12));
  newMap[i] = maskedNum | ((palIndex)<<12);
...
void deleteNode(NODE *head, NODE *n) {
 ...
 if(head == n) {
  ...
  while(prev->next != ((void *)0) && prev->next != n) {
   prev = prev->next;
  }
